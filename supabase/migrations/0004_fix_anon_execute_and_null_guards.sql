-- ==========================================================================
-- 0004: vá lỗ hổng nghiêm trọng phát hiện qua get_advisors:
--  1) Postgres tự cấp EXECUTE cho PUBLIC khi tạo function → role `anon`
--     (chưa đăng nhập) gọi được mọi RPC qua /rest/v1/rpc/*.
--  2) Với caller ẩn danh, auth.uid() = null → v_me = null → so sánh
--     "v_me.role != 'x'" cho kết quả NULL (không phải TRUE) → IF không
--     raise exception → guard bị lách qua hoàn toàn.
-- Vá: thêm chặn "phải đăng nhập" đầu mỗi hàm + REVOKE EXECUTE khỏi PUBLIC.
-- ==========================================================================
...
declare v_me profiles; v_mission missions; v_round int; v_sub_id uuid;
begin
...
-- ---- vá search_path mutable cho các hàm còn lại ----
create or replace function current_role_type() returns role_type language sql stable security definer set search_path = public as $$
...
begin
  ...
  if member_count >= 3 then raise exception 'Tiểu đội % đã đủ tối đa 3 thành viên phụ', new.squad_id; end if;
...
-- ---- REVOKE EXECUTE khỏi PUBLIC/anon cho MỌI RPC mutation — chỉ authenticated được gọi ----
revoke execute on function accept_mission(uuid) from public;
revoke execute on function submit_mission_result(uuid, jsonb) from public;
revoke execute on function approve_submission(uuid) from public;
revoke execute on function reject_submission(uuid, text) from public;
revoke execute on function revert_submission_to_rejected(uuid, text) from public;
revoke execute on function apply_penalty(uuid, text, text) from public;
revoke execute on function propose_commendation(uuid, text, text) from public;
revoke execute on function approve_commendation(uuid) from public;
revoke execute on function reject_commendation(uuid) from public;
revoke execute on function create_squad(text, text, uuid, uuid, front_type, text) from public;
revoke execute on function assign_squad_member(text, uuid, text) from public;
revoke execute on function create_mission(text, mission_type, uuid, uuid, numeric, text, int, text, text, boolean, text) from public;
revoke execute on function create_support_request(support_type, uuid, text) from public;
revoke execute on function respond_support_request(uuid, boolean) from public;
revoke execute on function cancel_support_request(uuid) from public;
revoke execute on function admin_create_warrior(uuid, text, text, text, front_type, role_type, text) from public;
revoke execute on function admin_set_active(uuid, boolean) from public;
revoke execute on function current_profile() from public;
-- hàm trigger nội bộ: không ai cần gọi trực tiếp qua RPC
revoke execute on function apply_exp_log_to_profile() from public;
revoke execute on function apply_exp_log_to_profile() from authenticated;
revoke execute on function check_squad_member_limits() from public;
revoke execute on function check_squad_member_limits() from authenticated;

-- re-grant đúng phạm vi: chỉ authenticated được gọi RPC nghiệp vụ
